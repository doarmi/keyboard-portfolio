...
const projectKeys: {
  ...
  polygon: string
}[] = [
    {
      ...
      name: 'PLIVY',
      slug: 'plivy',
      polygon: '1170,506 1203,498 1234,519 1243,540 1201,552 1181,534',
    },
    {
      key: 'D',
      name: 'DJSTAR',
      slug: 'djstar',
      polygon: '822,634 858,620 891,640 902,659 854,677 836,660',
    },
    {
      key: 'O',
      name: 'OASIS',
      slug: 'oasis',
      polygon: '1121,518 1156,510 1185,530 1196,551 1153,565 1134,547',
    },
  ]
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: self => {
          ...
          gsap.set(interaction, {
            opacity: isReady
              ? gsap.utils.mapRange(0.88, 0.96, 0, 1, self.progress)
              : 0,
  ...
  return (
    ...
    >
      ...
      >
        <button
          type="button"
          className="keyboard-sound-toggle"
        ...
        <div className="keyboard-scrub">
          <div
            ref={imageRef}
            className="keyboard-scrub-frame keyboard-photo-plane"
          >
            <img
              src={scrubImage}
              alt=""
              className="keyboard-photo-background"
              draggable={false}
            />

            <svg
              className={`keyboard-photo-keys ${ready ? 'is-ready' : ''
                }`}
              viewBox="0 0 1950 1096"
              preserveAspectRatio="xMidYMid slice"
            >
              <defs>
                {projectKeys.map(({ key, polygon }) => (
                  <clipPath
                    key={key}
                    id={`${clipId}-${key}`}
                  >
        ...
        <div
          ref={interactionRef}
          className="keyboard-interaction-layer"
        >
          <div
            ref={interactionRef}
            className="keyboard-interaction-layer"
          >
            <div
              className={`interaction-guide keyboard-only-guide keyboard-feedback-guide ${ready ? 'is-ready' : ''
                }`}
            >
              <strong>
                키보드의 P · D · O를 누르거나 화면의 키를 클릭하세요
              </strong>
